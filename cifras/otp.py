def _validar_chave(texto, chave_str):
    if not chave_str:
        raise ValueError("A chave não pode estar vazia.")
    
    if len(chave_str) < len(texto):
        raise ValueError(f"A chave do One-Time Pad deve ser pelo menos tão longa quanto o texto. (Texto: {len(texto)} chars, Chave: {len(chave_str)} chars)")
        
    return chave_str

def _operacao_otp_xor(texto, chave_str):
    chave = _validar_chave(texto, chave_str)
...
def cifrar(texto, chave_str):
    return _operacao_otp_xor(texto, chave_str)
...
def decifrar(texto_cifrado, chave_str):
    ...
    if len(chave) < len(texto_cifrado_bytes):
         raise ValueError(f"A chave do One-Time Pad deve ser pelo menos tão longa quanto o texto cifrado. (Texto: {len(texto_cifrado_bytes)} bytes, Chave: {len(chave)} chars)")
